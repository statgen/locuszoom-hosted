...
def warn_on_format_change(headers):
    ...
    assert headers == 'SNOMED_CID|SNOMED_FSN|SNOMED_CONCEPT_STATUS|UMLS_CUI|OCCURRENCE|USAGE|FIRST_IN_SUBSET|IS_RETIRED_FROM_SUBSET|LAST_IN_SUBSET|REPLACED_BY_SNOMED_CID\n', \
...
def to_json(rows):
    """
    Filter the set of SNOMED-CORE codes to some reusable subset, and serialize each one in a JSON format that will be recognized by Django loaddata
    """
    return [
        {
        ...
        if row[7] == 'False' # exclude retired
        and row[2] == 'Current'  # exclude outdated concepts
        #and row[4]  # usage stat info must be available...
        # and int(row[4]) >= 2  # ...and code must be used by >1 of the institutions tracked. We'll skip this rule for now as even 6k codes isn't awful.
    ]
